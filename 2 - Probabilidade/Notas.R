...
### Fenômeno Aleatório: é um processo de coleta de dados em que os resultados possíveis são conhecidos mas não se sabe qual deles ocorrerá. 
#### Exemplos:
...
### Espaço Amostral: O conjunto de todos os resultados possíveis do fenômeno aleatório. 
...
### Evento: Um subconjunto do espaço amostral Ω
#### Exemplo: Sejam os seguintes eventos no lançamento de um dado
#### A = {1,3,5}, B = {2,4,6}, C = {1,2}

##### O evento A ∩ B = Ø, é denominado [mutuamente excludente](https://pt.wikipedia.org/wiki/Eventos_mutuamente_exclusivos)
##### isto é, a ocorrência de A implica a não ocorrência de B e vice versa
###### Podemos concluir que A e B não possui intersecção

##### O evento A ∪ B = Ω, ou seja, temos um evento certo
###### que são todas as possibilidades do dado

##### OBS: ∪ (União) é equivalente ao operador lógico "ou", aumentando as possibilidades eventuais
...
### Probabilidade, P(.), é uma função que associa números reais aos elementos do espaço amostra
...
### Teorema da Soma
#### Possibilita a retirada de elementos que aparecem simultaneamente em mais de um evento 
##### (superestimam a possibilidade)
##### P(A ∪ B) = P(A) + P(B) - P (A ∩ B)
...
###### No lançamento de um dado, tendo A como 2
###### B sendo par, então temos que P(A|B) = 1/3
